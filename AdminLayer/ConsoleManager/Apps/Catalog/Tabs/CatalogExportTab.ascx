    ...
    <asp:UpdatePanel ID="FilesPanel" runat="server" UpdateMode="Conditional" ChildrenAsTriggers="true">
        <ContentTemplate>
            <table>
                <tr>
                    <td align="left">
                        <core:FileList ID="FilesControl" runat="server" Folder='<%# Mediachase.Web.Console.Common.ManagementHelper.GetImportExportFolderPath(_CatalogExportFolder) %>' GridAppId="Catalog" GridViewId="FilesList-Export" />
                    </td>
               </tr>
            </table>
        </ContentTemplate>
