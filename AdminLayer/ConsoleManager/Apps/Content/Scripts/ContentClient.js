...
function Mediachase_ContentClient()
{
    ...
    // -----------------  Language ----------------------
    this.NewLanguage = function(source) {
        CSManagementClient.ChangeView('Content', 'Language-Edit', '');
    };

    this.EditLanguage = function(langId) {
        CSManagementClient.ChangeView('Content', 'Language-Edit', 'langid=' + langId);
    };

    this.EditLanguage2 = function(params) {
        var langId = '';
        try {
            var cmdObj = Sys.Serialization.JavaScriptSerializer.deserialize(params);
            langId = cmdObj.CommandArguments.LangId;
        }
        catch (e) {
            alert('A problem occured with retrieving parameters for function EditLanguage2');
            return;
        }
        this.EditLanguage(langId);
    };

    this.LanguageSaveRedirect = function() {
        CSManagementClient.ChangeView('Content', 'Languages-List', '');
    };
};
