...
namespace Mediachase.Commerce.Manager.Core.MetaData.MetaControls
{
	...
	/// </summary>
    public partial class LongHtmlStringControl : CoreBaseUserControl, IMetaControl
	{
        /// <summary>
		...
		private void BindData()
		{
            if (MetaField != null && !MetaField.AllowNulls)
                MetaValueCtrl.IsRequired = true;
            else
                MetaValueCtrl.IsRequired = false;

            MetaLabelCtrl.Text = String.Format("{0} ({1})", MetaField.FriendlyName, LanguageCode);
            MetaValueCtrl.Text = MetaObject == null ? "" : MetaObject.GetString(MetaField);
