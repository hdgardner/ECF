            ...
            <tr>
		        <td>
                    <asp:datagrid id="ItemsGrid" runat="server" CssClass="Grid" AllowPaging="False" Width="100%"
	                    ShowFooter="False" DataKeyField="Key" AutoGenerateColumns="False">
	                    <ItemStyle CssClass="GridItem"></ItemStyle>
	                    <HeaderStyle CssClass="GridHeader"></HeaderStyle>
	                    <FooterStyle CssClass="GridFooter"></FooterStyle>
	                    <Columns>
		                    <asp:TemplateColumn HeaderText="<%$ Resources:SharedStrings, Key %>" HeaderStyle-Width="20%">
			                    <ItemTemplate>
				                    <asp:Literal ID="txtKey" Runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "Key")%>'></asp:Literal>
			                    </ItemTemplate>
		                    </asp:TemplateColumn>
		                    <asp:TemplateColumn HeaderText="<%$ Resources:SharedStrings, Value %>" HeaderStyle-Width="70%" >
			                    <ItemTemplate>
				                    <%# DataBinder.Eval(Container.DataItem, "Value")%>
			                    </ItemTemplate>
			                    <EditItemTemplate>
				                    <asp:TextBox Runat="server" ID="tbValue" Text='<%# DataBinder.Eval(Container.DataItem, "Value")%>'></asp:TextBox>
		                    ...
		                    <asp:TemplateColumn HeaderText="<%$ Resources:SharedStrings, Options %>" HeaderStyle-Width="60" ItemStyle-HorizontalAlign="Center">
			                    <ItemTemplate>
