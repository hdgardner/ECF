...
function CheckTime()
{
  if(!fl && rsslastTime)
  {
    ...
    {
      document.getElementById('divNews').innerHTML = "<div style='text-align:center;padding:10px;color:red;' class='text'>"+'<%= RM.GetString("RSS_ERROR")%>'+"</div>";
      return;
    }
    else
    {
...
function CreateRssNews(feedUrl, feedCount, containerId)
{
  ...
  document.getElementById(containerId).innerHTML = "<div style='padding:10px;text-align:center;'><img alt='' align='absmiddle' border='0' src='"+'<%= Page.ResolveClientUrl("~/App_Themes/Default/images/Shell/loading_small.gif") %>' + "'></div>";
  var ajaxRequest;
