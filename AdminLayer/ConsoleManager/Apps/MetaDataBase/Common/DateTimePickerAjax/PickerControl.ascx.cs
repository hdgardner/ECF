...
namespace Mediachase.Ibn.Web.UI
{
	public partial class PickerControl : System.Web.UI.UserControl
	{
		...
		internal FirstDayOfWeek _firstDayOfWeek = FirstDayOfWeek.Monday; //(FirstDayOfWeek)Company.GetCompanyFirstDayOfWeek();
		...
		public DateTime SelectedDate
		{
			get
			{
				...
				if (SelectedMode == SelectedModeType.Week)
				{
					if (!String.IsNullOrEmpty(txtDate.Text))
						txtDate.Text = CHelper.GetWeekStartByDate(retVal).ToString(this.DateFormat, CultureInfo.CurrentCulture.DateTimeFormat) + " - " + CHelper.GetWeekStartByDate(retVal).AddDays(6).ToString(this.DateFormat, CultureInfo.CurrentCulture.DateTimeFormat);
					return CHelper.GetWeekStartByDate(retVal);
				}
			...
			set
			{
				...
				if (SelectedMode == SelectedModeType.Week)
				{
					retVal = CHelper.GetWeekStartByDate(value);
		...
		public string ButtonImageUrl
		{
			get
			{
				if (ViewState[this.ID + "_ButtonImageUrl"] != null)
					...
					return "~/Apps/MetaDataBase/Images/mcCalendar.gif";
			}
			set
			{
		...
		public string ResetImageUrl
		{
			get
			{
				if (ViewState[this.ID + "_ResetImageUrl"] != null)
					...
					return "~/Apps/MetaDataBase/Images/reset17.gif";
			}
			set
			{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			calendarButtonExtender.FirstDayOfWeek = _firstDayOfWeek;
			...
			this.Page.ClientScript.RegisterClientScriptBlock(this.GetType(), Guid.NewGuid().ToString("N"),
				String.Format("<link rel='Stylesheet' href='{0}' type='text/css' />",
					CHelper.GetAbsolutePath("/Apps/MetaDataBase/styles/calendar.css")));
		}
