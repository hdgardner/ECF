	...
	public partial class MetaFieldEdit : System.Web.UI.UserControl, IAutogenerateSystemNames
	{
		...
		public string ClassLabelText
		{
			get 
			{
				...
				{
					if (mc.IsCard)
						retval = GetGlobalResourceObject("GlobalMetaInfo", "Card").ToString();
					else if (mc.IsBridge)
						retval = GetGlobalResourceObject("GlobalMetaInfo", "Bridge").ToString();
					else
						retval = GetGlobalResourceObject("GlobalMetaInfo", "Info").ToString();
				}
				else
				{
		...
		private void BindInfo()
		{
			if (mf != null)
			{
				MainBlockHeader.Title = GetGlobalResourceObject("GlobalMetaInfo", "EditField").ToString();
			}
			else
			{
				MainBlockHeader.Title = GetGlobalResourceObject("GlobalMetaInfo", "NewField").ToString();
			...
			MainBlockHeader.AddLink(
				String.Format(CultureInfo.InvariantCulture, "<img src='{0}' border='0' align='absmiddle' width='16px;' height='16px;' />&nbsp;{1}", ResolveClientUrl("~/Apps/MetaDataBase/Images/cancel.gif"), GetGlobalResourceObject("GlobalMetaInfo", "Back").ToString()),
