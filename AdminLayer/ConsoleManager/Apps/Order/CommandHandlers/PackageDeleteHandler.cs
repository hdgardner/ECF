...
{
	public class PackageDeleteHandler : ICommand
	{
		...
		void ProcessDeleteCommand(string[] items)
		{
			for (int i = 0; i < items.Length; i++)
			{
				...
				if (keys != null)
				{
					...
					ShippingMethodDto dto = ShippingManager.GetShippingPackage(id);
					if (dto != null && dto.Package.Count > 0)
					{
						dto.Package[0].Delete();
						ShippingManager.SavePackage(dto);
						// related shipping package will be deleted acoording to db constraints
					}
				}
			}
		}
