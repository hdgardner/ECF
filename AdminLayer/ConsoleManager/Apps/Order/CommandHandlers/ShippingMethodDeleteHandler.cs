...
{
	public class ShippingMethodDeleteHandler : ICommand
	{
		...
		void ProcessDeleteCommand(string[] items)
		{
			for (int i = 0; i < items.Length; i++)
			{
				...
				if (keys != null)
				{
					...
					// delete selected shipping method
					ShippingMethodDto dto = ShippingManager.GetShippingMethod(id, true);
					if (dto != null && dto.ShippingMethod.Count > 0)
					{
						dto.ShippingMethod[0].Delete();
