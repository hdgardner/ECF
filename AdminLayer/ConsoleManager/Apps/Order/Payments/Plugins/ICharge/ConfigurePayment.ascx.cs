...
namespace Mediachase.Commerce.Manager.Order.Payments.Plugins.ICharge
{
	/// <summary>
    ...
    public partial class ConfigurePayment : OrderBaseUserControl, IGatewayControl
	{
		...
		{
            XmlTextReader reader = new XmlTextReader(Server.MapPath("~/Apps/Order/Payments/Plugins/ICharge/ConfigParams.xml"));
			XmlSerializer mySerializer = new XmlSerializer(typeof(GatewayManager));
			GatewayManager gm = (GatewayManager)mySerializer.Deserialize(reader);
			reader.Close();
			return gm;
		}
