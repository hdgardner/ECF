...
{
	/// <summary>
	///		Summary description for ShippingOptionEditParameters.
	/// </summary>
	public partial class ShippingOptionEditParameters : OrderBaseUserControl, IAdminTabControl, IAdminContextControl
	{
		...
		private void BindShippingConfig()
		{
			...
			if (_ShippingMethodDto != null && _ShippingMethodDto.ShippingOption.Count > 0)
			{
				try
				{
					...
					String mainPath = string.Concat(_ShippingGatewayConfigurationBasePath, shippingRow.SystemKeyword, _ShippingGatewayConfigurationFileName);
					...
					if (ctrl != null)
					{
						ctrl.ID = shippingRow.SystemKeyword;
						IGatewayControl tmpCtrl = (IGatewayControl)ctrl;
						tmpCtrl.LoadObject(_ShippingMethodDto);
						tmpCtrl.ValidationGroup = "vg" + shippingRow.SystemKeyword;
