    ...
    <asp:UpdatePanel ID="FilesPanel" runat="server" UpdateMode="Conditional" ChildrenAsTriggers="true">
        <ContentTemplate>
            <table>
                <tr>
                    <td align="left">
                        <core:FileList ID="FilesControl" runat="server" Folder='<%# Mediachase.Web.Console.Common.ManagementHelper.GetImportExportFolderPath(_SiteExportFolder) %>' GridAppId="Content" GridViewId="FilesList-Export" />
                    </td>
               </tr>
            </table>
        </ContentTemplate>
