<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CheckoutSummary.ascx.cs" Inherits="Mediachase.Cms.Website.Structure.User.NWTDControls.Controls.Cart.CheckoutSummary" %>
...
<asp:Panel runat="server" ID="pnlOrderSubmitted" Visible="false">
	...
	<p>
		You can get status information regarding your order by logging into our website and selecting 'Order Status' from the Quick Links section.  Order status is typically available 24-48 hours after you place your order.  If you have any questions regarding this order or need to make changes, please contact us.
	</p>
	<p>
...
--%>
	<%--On 08/02/17, Heath Gardner moved the wishlist id from the bottom of the req summary here to the title (lblTitle value set in Code Behind)--%>
    <h1 class="cart-header"><asp:Literal runat="server" ID="lblTitle"></asp:Literal></h1>
		...
		<table cellpadding="0" cellspacing="0" border="0" width="100%">
			...
			<tr>
				<td>
					...
					<!-- On 08/02/17, Heath Gardner removed 'this.CheckoutCart.GetString("OrderFax")' per Scott Seeley's and Carrie R's request -->                   
						<tr>
	...
	<asp:Panel runat="server" ID="pnlLevelBNotifications" CssClass="order-summary-notification-panel">
		<p>To place your order, print your requisition and fax, mail, or e-mail it to us along with a purchase order.</p>
        <p><b>All no-charge items are subject to approval.<br />Prices are set by our publishers and are subject to change.</b></p> 
	</asp:Panel>
    <!-- On 08/02/17, Heath Gardner moved the WishList ID & name from the above 'panel' to the top of this summary page per Scott Seeley's and Carrie R's request -->
    ...
    <!-- On 03/01/18, Heath Gardner added the following Price disclaimer per Customer Service's request and un-bolded the No Charge disclaimer per Scott S.'s request -->
	<asp:Panel runat="server" ID="pnlLevelANotifications" CssClass="order-summary-notification-panel" Visible="false">
		<p class="shipping-rate-notice">*Shipping rates are based on standard delivery charges. <br /> <br /> Prices are set by the publisher and are subject to change. <br /> All no-charge items are subject to approval. </p>
	</asp:Panel>
