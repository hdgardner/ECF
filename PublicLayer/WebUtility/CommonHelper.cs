...
namespace Mediachase.Cms.Util
{
    ...
    public class CommonHelper
    {
		...
		internal const string _IsDesignModeString = "IsDesignMode";
		internal const string _MediachaseCMSDesignCookieString = "MediachaseCMSDesign";
        ...
        {
            Account account = ProfileContext.Current.GetAccount(userGuid.ToString());
        ...
        /// <param name="list">The list.</param>
        public static void SaveParameter(ControlSettings settings, string paramName, ListBox list)
        {
            settings.Params.Remove(paramName);

            string savedCommaString = String.Empty;
            foreach (ListItem item in list.Items)
            {
                if (item.Selected)
                {
                    if (String.IsNullOrEmpty(savedCommaString))
                        savedCommaString = item.Value;
                    else
                        savedCommaString = savedCommaString + "," + item.Value;
                }
            }
            if (!String.IsNullOrEmpty(savedCommaString))
                settings.Params.Add(paramName, savedCommaString);
        }
        ...
        public static void LoadTextBox(ControlSettings settings, string paramName, TextBox textbox)
        {
            if ((settings.Params[paramName] != null) && (settings.Params[paramName] is string))
            {
                textbox.Text = settings.Params[paramName].ToString();
            }
        }
        ...
        public static string GetFlagIcon(CultureInfo culture)
        {
			if (!String.IsNullOrEmpty(culture.Name) && culture.Name.Length >= 4)
			{
				...
				string path = String.Format("~/images/Flags/{0}.gif", extension);
				...
				path = String.Format("~/images/Flags/{0}.gif", extension);
				if (File.Exists(HttpContext.Current.Server.MapPath(path)))
					return path;
			}
		...
		public static string GetImageUrl(string relativeUrl, Page page)
		{
			...
			string path = page.MapPath(String.Format("~/App_Themes/{0}/{1}", theme, relativeUrl));

			if (System.IO.File.Exists(path)) // try current theme
				imageUrl = String.Format("~/App_Themes/{0}/{1}", theme, relativeUrl);
			else // try default theme
				imageUrl = String.Format("~/App_Themes/{0}/{1}", "Default", relativeUrl);
		...
		public static string MakeCurrentApplicationCookieName(string baseName)
		{
			...
			if (!String.IsNullOrEmpty(path))
				return baseName + CmsConfiguration.Instance.ApplicationId + path.Replace('/', '_');
			else
				return baseName;
		}
		#endregion
		...
		public static bool CheckDesignMode(HttpContext context)
		{
			bool retVal = false;
			if (!String.IsNullOrEmpty(context.Request.Form[_IsDesignModeString]))
			{
				Boolean.TryParse(context.Request.Form[_IsDesignModeString], out retVal);
				// if "Design"/"Public" tab was clicked, update the cookie
				HttpCookie cookie = new HttpCookie(CommonHelper._MediachaseCMSDesignCookieString);
				if (String.Compare(cookie.Value, retVal.ToString()) != 0)
				{
					cookie.Expires = DateTime.MaxValue;
					cookie.Value = retVal.ToString();
					context.Response.Cookies.Add(cookie);
				}
			}
			else
			{
				if (context.Request.Cookies[_MediachaseCMSDesignCookieString] != null)
				{
					HttpCookie cookie = (HttpCookie)context.Request.Cookies[_MediachaseCMSDesignCookieString];
					Boolean.TryParse(cookie.Value, out retVal);
				}
			}
