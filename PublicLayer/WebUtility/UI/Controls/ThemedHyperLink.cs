namespace Mediachase.Cms.Web.UI.Controls
{
    ...
    /// </summary>
    public partial class ThemedHyperLink : HyperLink
    {
        /// <summary>
        ...
        protected override void OnPreRender(EventArgs e)
        {
            if (!this.Page.IsPostBack || !this.IsViewStateEnabled)
            {
                ...
                if (System.IO.File.Exists(path)) // try current theme
                    this.ImageUrl = this.Page.ResolveUrl(String.Format("~/App_Themes/{0}/{1}", theme, ImageUrl));
                else // try default theme
                    this.ImageUrl = this.Page.ResolveUrl(String.Format("~/App_Themes/{0}/{1}", "Default", ImageUrl));
            }
            base.OnPreRender(e);
        }
    }
